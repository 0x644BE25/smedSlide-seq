...
# expression data :)
seu$seurat_clusters <- droplevels(factor(meta[Cells(seu),'global.cluster'],levels=0:99))
seu$tissue <- factor(meta[Cells(seu),'tissue'],levels=tissuelvs)
umap <- as.matrix(meta[Cells(seu),c('global.umap.x','global.umap.y')])
...
# ALSO: piwi1+ subcluster numbers are off by 1 because 
# Seurat likes to start at 0 and 
# R likes to start at 1 and 
# I wasn't careful
# oops.
seu$seurat_clusters <- droplevels(factor(as.integer(meta[Cells(seu),'piwi.positive.subcluster'])-1,levels=0:99))
seu$tissue <- factor(meta[Cells(seu),'tissue'],levels=tissuelvs)
umap <- apply(meta[Cells(seu),c('piwi.positive.umap.x','piwi.positive.umap.y')],2,as.numeric)
rownames(umap) <- Cells(seu)
colnames(umap) <- c('umap_1','umap_2')
seu[['umap']] <- CreateDimReducObject(umap,key='umap_')
